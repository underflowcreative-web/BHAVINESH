  ...
  return (
    <>
      ...
      <section className="relative h-[50vh] sm:h-[60vh] md:h-[70vh] flex items-end overflow-hidden">
        ...
        <div className="relative section-padding pb-10 sm:pb-16 w-full">
          <ScrollReveal>
            <Link
              href="/#categories"
              className="text-label text-stone/60 hover:text-bronze transition-colors duration-300 mb-4 sm:mb-6 inline-block nav-link-premium min-h-[38px] flex items-center"
            >
              ← Back to Categories
            </Link>
            <h1 className="text-section-title text-[#e8e4df]">{category.name}</h1>
            <p className="text-body-lg mt-2 sm:mt-4 max-w-lg text-xs sm:text-base">{category.description}</p>
            <p className="text-label text-stone/40 mt-3 sm:mt-4 text-[11px] sm:text-xs">{images.length} Photographs</p>
          </ScrollReveal>
        </div>
      </section>
      ...
      <section className="section-padding editorial-gap">
        <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-2.5 sm:gap-3 md:gap-4">
          {images.map((image, index) => (
            <ScrollReveal key={image.id} delay={Math.min(index * 0.05, 0.4)}>
              <div
                className="portfolio-image-wrapper mb-2.5 sm:mb-3 md:mb-4 break-inside-avoid rounded-sm"
              ...
              >
                <motion.img
                  ...
                  style={{
                    height: index % 3 === 0 ? "380px" : index % 3 === 1 ? "280px" : "320px",
                  }}
                  loading="lazy"
      ...
      {/* Gallery End CTA */}
      <section className="section-padding py-16 sm:py-20 md:py-28 bg-primary relative">
        ...
        <div className="relative z-10 max-w-2xl mx-auto text-center space-y-4 sm:space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-3 sm:gap-4 pt-2">
            <a
              ...
              className="uppercase tracking-[0.2em] sm:tracking-[0.22em] font-heading text-xs bg-[#8B7355] hover:bg-[#8B7355]/80 text-white px-7 sm:px-8 py-3.5 transition-all duration-300 rounded-sm font-medium inline-flex items-center justify-center gap-2 min-h-[46px]"
            >
            ...
            <a
              ...
              onMouseEnter={() => setCursorVariant('button')}
              onMouseLeave={() => setCursorVariant('default')}
              className="uppercase tracking-[0.2em] sm:tracking-[0.22em] font-heading text-xs border border-white/20 hover:border-[#8B7355] text-[#e8e4df] hover:text-[#8B7355] px-7 sm:px-8 py-3.5 transition-all duration-300 rounded-sm inline-flex items-center justify-center gap-2 min-h-[46px]"
            >
